# PROJETO EM PYGAME GRUPO QC TI UNIVERSIDADE UNIGRANRIO 
# ESTE PROJETO ESTA SENDO USADO PARA ESTUDO DA LINGUAGEM PYTHON
# VERSAO 1.1
...
def draw(): # Função escrevendo na tela tamanho da fonte cores etc...
    screen.blit('background', (0, 0))
    if gameStatus == 0: # exibição do titulo da página
        drawCentreText(" COVID-19 INVADERS\n Fiquem em casa e brinquem com o nosso jogo\n\nDigite seu nome\nAperte Enter para Iniciar \n(Use as setas para movimentar e tecla espaço para tiro)")
        screen.draw.text(player.name , center=(400, 500), owidth=0.5, ocolor=(255,0,0), color=(0,64,255) , fontsize=40)
    if gameStatus == 1:
        ...
        screen.draw.text("NIVEL " + str(level) , midtop=(400, 10), owidth=0.5, ocolor=(255,255,255), color=(0,64,255) , fontsize=40)
        drawLives()
        if player.status >= 30:
            if player.lives > 0:
                drawCentreText("VOCÊ FOI ABATIDO!\nPressione  Enter para jogar de novo")
            else:
                drawCentreText(" O JOGO ACABOU!\nPressione Enter para continuar")
        if len(aliens) == 0 :
            drawCentreText("PASSOU DE NÍVEL! LEMBRE-SE! USE A MASCARA AO SAIR DE CASA\nPressione Enter para ir para o próximo nível")
    if gameStatus == 2: # Jogo acabou exibe a tabela de classificação
        drawHighScore()

def drawCentreText(t):
    screen.draw.text(t , center=(400, 300), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=30)
...
def readHighScore(): # Função que lê o arquivo txt e as pontuações dos jogadores
...
def writeHighScore(): # Função que guardamos as pontuações dos jogadores no txt
    global highScore
...
def drawHighScore():
    global highScore
    y = 0
    screen.draw.text("MELHORES PONTOS", midtop=(400, 30), owidth=0.5, ocolor=(255,255,255), color=(0,64,255) , fontsize=60)
    for line in highScore:
        if y < 400:
            screen.draw.text(line, midtop=(400, 100+y), owidth=0.5, ocolor=(0,0,255), color=(255,255,0) , fontsize=50)
            y += 50
    screen.draw.text("Pressione Escape para jogar de novo" , center=(400, 550), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=60)
...
def updateCovid(): # Configurando a dificuldade
...
def updateBoss(): # função adicionando o chefe
...
def init(): # Função numero de vidas do jogador
